...
async def add_event(info: Dict):
    ...
    event.begin = info["date"] - timedelta(
        hours=4
    )  # -4 так как в россии UTC + 4
    event.duration = timedelta(hours=1)
    event.uid = info["uid"]
    event.description = (
        f"Сотрудник: {info['worker_name']}\n" f"Клиент: {client_name}"
    )
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.put(
            event_url, data=ics_data, headers=headers, ssl=ssl_context
        ) as resp:
...
async def delete_event(uid: str = "1"):
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.delete(
            event_url, headers=headers, ssl=ssl_context
        ) as resp:
            logging.info(f"Статус ответа на DELETE: {resp.status}")
            if not resp.status in (200, 204):
                logging.info("Событие удалено успешно!")
            elif resp.status == 404:
                logging.error("Событие не найдено.")
            else:
                text = await resp.text()
                logging.error(text)
